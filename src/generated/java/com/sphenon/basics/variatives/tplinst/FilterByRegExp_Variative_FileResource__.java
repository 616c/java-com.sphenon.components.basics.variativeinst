...
public class FilterByRegExp_Variative_FileResource__ extends GenericFilterBase<Variative_FileResource_> implements FilterByExpression_Variative_FileResource__ {
    ...
    public FilterByRegExp_Variative_FileResource__ (CallContext context) {
        super(context, target_type == null ? (target_type = TypeManager.get(context, Variative_FileResource_.class)) : target_type);
    }

    public FilterByRegExp_Variative_FileResource__ (CallContext context, String regexp) throws PatternSyntaxException {
        this(context);
    ...
    public boolean matches (CallContext context, Variative_FileResource_ object) {
        Variative_FileResource_ instance = object;
        return ( this.regexp != null
                 ? (    this.regexp.length() == 0
                     || this.regexp_pattern == null
                     || (object != null && regexp_pattern.matcher(instance.getVariant_FileResource_(context).getName(context) ).matches())
                   ) :
                 this.value != null
                 ? (object != null && this.value.equals(instance.getVariant_FileResource_(context).getName(context) )
                   ) :
                 true);
    }

    static public FilterByRegExp_Variative_FileResource__ newInstance(CallContext context) {
        return (FilterByRegExp_Variative_FileResource__) Factory_Filter_Variative_FileResource__.construct(context);
    }
}
