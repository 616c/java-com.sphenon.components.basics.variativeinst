...
public class FilterByRegExp_Variative_Image__ extends GenericFilterBase<Variative_Image_> implements FilterByExpression_Variative_Image__ {
    ...
    public FilterByRegExp_Variative_Image__ (CallContext context) {
        super(context, target_type == null ? (target_type = TypeManager.get(context, Variative_Image_.class)) : target_type);
    }

    public FilterByRegExp_Variative_Image__ (CallContext context, String regexp) throws PatternSyntaxException {
        this(context);
    ...
    public boolean matches (CallContext context, Variative_Image_ object) {
        Variative_Image_ instance = object;
        return ( this.regexp != null
                 ? (    this.regexp.length() == 0
                     || this.regexp_pattern == null
                     || (object != null && regexp_pattern.matcher(instance.getVariant_Image_(context).getName(context) ).matches())
                   ) :
                 this.value != null
                 ? (object != null && this.value.equals(instance.getVariant_Image_(context).getName(context) )
                   ) :
                 true);
    }

    static public FilterByRegExp_Variative_Image__ newInstance(CallContext context) {
        return (FilterByRegExp_Variative_Image__) Factory_Filter_Variative_Image__.construct(context);
    }
}
