...
public class FilterBuilder_Variative_Object__ implements GenericFilterBuilder<Variative_Object_> {

    public FilterBuilder_Variative_Object__ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Variative_Object_.class)) : target_type);
    }
    ...
    protected GenericFilter<Variative_Object_> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Variative_Object_> filter = Factory_Filter_Variative_Object__.construct(context);

        if ((filter instanceof FilterByExpression_Variative_Object__) == false || operator == null || operator.matches("[=~]") == false) {
                               ...
                               ((FilterByExpression_Variative_Object__) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Variative_Object_>> getter;
    protected Setter<GenericFilter<Variative_Object_>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Variative_Object_>> getter, Setter<GenericFilter<Variative_Object_>> setter, Creator<GenericFilter<Variative_Object_>> creator) {
        this.getter = getter;
        this.setter = setter;
