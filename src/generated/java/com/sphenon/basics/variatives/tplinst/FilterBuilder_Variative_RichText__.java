...
public class FilterBuilder_Variative_RichText__ implements GenericFilterBuilder<Variative_RichText_> {

    public FilterBuilder_Variative_RichText__ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Variative_RichText_.class)) : target_type);
    }
    ...
    protected GenericFilter<Variative_RichText_> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Variative_RichText_> filter = Factory_Filter_Variative_RichText__.construct(context);

        if ((filter instanceof FilterByExpression_Variative_RichText__) == false || operator == null || operator.matches("[=~]") == false) {
                               ...
                               ((FilterByExpression_Variative_RichText__) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Variative_RichText_>> getter;
    protected Setter<GenericFilter<Variative_RichText_>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Variative_RichText_>> getter, Setter<GenericFilter<Variative_RichText_>> setter, Creator<GenericFilter<Variative_RichText_>> creator) {
        this.getter = getter;
        this.setter = setter;
