...
public class VectorImpl_Variant_Object__long_
  implements Vector_Variant_Object__long_,
             VectorOptimized<Variant_Object_>,
             Dumpable,
             ManagedResource
 {
    private java.util.Vector vector;

    protected VectorImpl_Variant_Object__long_ (CallContext context) {
        vector = new java.util.Vector(4);
    }

    static public VectorImpl_Variant_Object__long_ create (CallContext context) {
        return new VectorImpl_Variant_Object__long_(context);
    }

    protected VectorImpl_Variant_Object__long_ (CallContext context, java.util.Vector vector) {
        this.vector = vector;
    }

    static public VectorImpl_Variant_Object__long_ create (CallContext context, java.util.Vector vector) {
        return new VectorImpl_Variant_Object__long_(context, vector);
    }
    ...
    public Variant_Object_ get          (CallContext context, long index) throws DoesNotExist {
        try {
            return (Variant_Object_) vector.elementAt((int) index);
    ...
    public void     prepend      (CallContext call_context, Variant_Object_ item) {
        if (vector.size() == 0) {
            vector.add(item);
        } else {
            try {
    ...
    public Variant_Object_ replace      (CallContext call_context, long index, Variant_Object_ item) throws DoesNotExist {
        try {
            return (Variant_Object_) vector.set((int) index, item);
        } catch (ArrayIndexOutOfBoundsException e) {
    ...
    public boolean contains(CallContext context, Variant_Object_ item) {
        return this.vector.contains(item);
    }

    public boolean removeFirst(CallContext context, Variant_Object_ item) {
        return this.vector.remove(item);
    }

    public void removeAll(CallContext context, Variant_Object_ item, VectorOptimized.Notifier<Variant_Object_> notifier) {
