...
public class FilterBuilder_Variative_String__ implements GenericFilterBuilder<Variative_String_> {

    public FilterBuilder_Variative_String__ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Variative_String_.class)) : target_type);
    }
    ...
    protected GenericFilter<Variative_String_> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Variative_String_> filter = Factory_Filter_Variative_String__.construct(context);

        if ((filter instanceof FilterByExpression_Variative_String__) == false || operator == null || operator.matches("[=~≈]") == false) {
                               ...
                               ((FilterByExpression_Variative_String__) filter).setExpression(context, (operator.matches("[~≈]") ? operator : "") + value);
    ...
    protected Getter<GenericFilter<Variative_String_>> getter;
    protected Setter<GenericFilter<Variative_String_>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Variative_String_>> getter, Setter<GenericFilter<Variative_String_>> setter, Creator<GenericFilter<Variative_String_>> creator) {
        this.getter = getter;
        this.setter = setter;
