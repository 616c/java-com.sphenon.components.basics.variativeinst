...
public class VectorObserver_SubTransactionLifeCycleLinked_long_
  implements Vector_SubTransactionLifeCycleLinked_long_,
             Changing,
             Dumpable,
             ManagedResource
{
    protected Vector_SubTransactionLifeCycleLinked_long_ observed_vector;
    ...
    public VectorObserver_SubTransactionLifeCycleLinked_long_ (CallContext context, Vector_SubTransactionLifeCycleLinked_long_ observed_vector,VectorObserverEventPolicy event_policy, ModificationType modification_type) {
        ...
        if (this.observed_vector != null && modification_type != ModificationType.STATE_LOAD) {
            if (getEventPolicy(context) == VectorObserverEventPolicy.ON_DEMAND) {
                ...
                for (SubTransactionLifeCycleLinked item : observed_vector.getIterable_SubTransactionLifeCycleLinked_(context)) {
                    onAdd(context, item);
                }
            }
        }
    ...
    public VectorObserver_SubTransactionLifeCycleLinked_long_ (CallContext context, Vector_SubTransactionLifeCycleLinked_long_ observed_vector,VectorObserverEventPolicy event_policy) {
        this(context, observed_vector, event_policy, null);
    }

    public VectorObserver_SubTransactionLifeCycleLinked_long_(CallContext context, Vector_SubTransactionLifeCycleLinked_long_ observed_vector) {
        this(context, observed_vector, VectorObserverEventPolicy.ON_DEMAND);   
    }

    public void setObserved (CallContext context, Vector_SubTransactionLifeCycleLinked_long_ observed_vector) {
        if (this.getObserved(context) != null) {
            for (SubTransactionLifeCycleLinked item : this.observed_vector.getIterable_SubTransactionLifeCycleLinked_(context)) {
    ...
    public Vector_SubTransactionLifeCycleLinked_long_ getObserved (CallContext context) {
    ...
    protected void onAdd(CallContext context, SubTransactionLifeCycleLinked item) {
    }

    protected void onRemove(CallContext context, SubTransactionLifeCycleLinked item) {
    }
    ...
    public SubTransactionLifeCycleLinked                                    set             (CallContext context, long index, SubTransactionLifeCycleLinked item) {
        SubTransactionLifeCycleLinked old_item = this.getObserved(context).set(context, index, item);
        if (old_item != null) { 
    ...
    public SubTransactionLifeCycleLinked                                    replace         (CallContext context, long index, SubTransactionLifeCycleLinked item) throws DoesNotExist {
        SubTransactionLifeCycleLinked old_item = this.getObserved(context).replace(context, index, item);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        onAdd(context, item);
    ...
    public SubTransactionLifeCycleLinked                                    unset           (CallContext context, long index) {
        SubTransactionLifeCycleLinked old_item = this.getObserved(context).unset(context, index);
        if (old_item != null) { 
           onRemove(context, old_item); 
           ChangeEvent event = new RemoveEvent(context,old_item,index);   
           notify(context,event);
    ...
    public SubTransactionLifeCycleLinked                                    remove          (CallContext context, long index) throws DoesNotExist {
        SubTransactionLifeCycleLinked old_item = this.getObserved(context).remove(context, index);
        onRemove(context, old_item);
        ChangeEvent event = new RemoveEvent(context,old_item,index);   
        notify(context,event);
        return old_item;
    ...
    public java.util.Iterator<SubTransactionLifeCycleLinked> getIterator_SubTransactionLifeCycleLinked_ (CallContext context) {
        return this.getObserved(context).getIterator_SubTransactionLifeCycleLinked_(context);
    }
