...
public class FilterBuilder_Variative_FileResource__ implements GenericFilterBuilder<Variative_FileResource_> {

    public FilterBuilder_Variative_FileResource__ (CallContext context) {
    }

    static protected Type target_type;

    public Type getTargetType (CallContext context) {
        return (target_type == null ? (target_type = TypeManager.get(context, Variative_FileResource_.class)) : target_type);
    }
    ...
    protected GenericFilter<Variative_FileResource_> createFilter(CallContext context, String unique_filter_id, String operator, Object value) throws InvalidQueryExpression {
        GenericFilter<Variative_FileResource_> filter = Factory_Filter_Variative_FileResource__.construct(context);

        if ((filter instanceof FilterByExpression_Variative_FileResource__) == false || operator == null || operator.matches("[=~]") == false) {
                               ...
                               ((FilterByExpression_Variative_FileResource__) filter).setExpression(context, (operator.matches("~") ? "~" : "") + value);
    ...
    protected Getter<GenericFilter<Variative_FileResource_>> getter;
    protected Setter<GenericFilter<Variative_FileResource_>> setter;

    protected String member_name;

    public GenericFilterBuilder wireToSimpleMember(CallContext context, String member_name, Getter<GenericFilter<Variative_FileResource_>> getter, Setter<GenericFilter<Variative_FileResource_>> setter, Creator<GenericFilter<Variative_FileResource_>> creator) {
        this.getter = getter;
        this.setter = setter;
